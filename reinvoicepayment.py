...
class InvoicePaymentRe(QMainWindow):
	def __init__(self,data,date1,date2, parent=None):
		super(InvoicePaymentRe, self).__init__(parent)
	...
	def Preview(self):
		loaddata=open("db/InvoicePaymentRe.json", "r")
	...
	def handle_paint_request(self, printer):
		...
		headers =["Date","Invoice","Customer","Method","status","Reference","Amount"]
		data=self.printdata['invoicepaymentdata']
		subtotal=self.printdata['subtotal']
      ...
      <p>Invoices Payment Report<br>Period:{{date1}} to {{date2}} <p/>
     </div>
	...
	def GeneralInvoive(self):
		...
		self.invoicepaymentdata=self.data['invoicepaymentdata']
		subno=len(self.data['subtotal'])
		self.invoiceno=(len(self.invoicepaymentdata)-subno-2)
		trows=''
		
		totalamount=self.data['total'][1]
		subtotalt=self.data['subtotal']
		for row in sorted(self.invoicepaymentdata):
			date=self.invoicepaymentdata[row][0]
			invoice=self.invoicepaymentdata[row][1]
			customer=self.invoicepaymentdata[row][2]
			method=self.invoicepaymentdata[row][3]
			reference=self.invoicepaymentdata[row][4]
			amount=self.invoicepaymentdata[row][5]
			...
			<td style="border: 1px solid #dddddd; text-align: left;padding: 8px;">{customer}</td><td style="border: 1px solid #dddddd; text-align: left;padding: 8px;">{method}</td>
			...
			'''.format(date=date,invoice=invoice,customer=customer,method=method,reference=reference,amount=amount)
			trows=trows + tr
		...
		table= '''
	        ...
	        <td style="border: 1px solid #dddddd; text-align: left;padding: 8px;font-weight: bold;">Customer</td><td style="border: 1px solid #dddddd; text-align: left;padding: 8px;font-weight: bold;">Method</td>
	        <td style="border: 1px solid #dddddd; text-align: left;padding: 8px;font-weight: bold;">Reference</td>
		...
		printdata=open("db/InvoicePaymentRe.json", "w")
