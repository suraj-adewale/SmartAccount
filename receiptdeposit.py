from PyQt5.QtWidgets import QPushButton,QFormLayout,QListWidget,QDialogButtonBox,QDialog,QAbstractItemView,QTableWidget,QTextEdit,QLabel,QCompleter,QTableWidgetItem,\
...
class ReceiptDeposit(QMainWindow):
	def __init__(self,edit_data, parent=None):
		super(ReceiptDeposit, self).__init__(parent)
		self.title = 'Receipts and Deposits'
	...
	def addJournalContent(self):
		...
		requireddata2=open("db/cashaccounts.json", "r")
		...
		deposit=QLabel("Deposit account:")
		...
		self.memo.setText("Receipt")
		self.balance.setReadOnly(True)
		
		self.deposit.currentTextChanged.connect(self.DepositChange)	
		self.cashaccountdata=self.requireddata2
		self.deposit.addItem("")
		self.deposit.insertItem(0,"-- Create new account --")
		
		for key in sorted(self.cashaccountdata):
			self.deposit.insertItem(int(key)+1,self.cashaccountdata[key][1])
		...
		try:
			self.template=json.load(open("db/receiptdeposit_template.json","r"))
			self.templatebutton.setText("Use Template {}".format(len(self.template)))
		except Exception as e:
			json.dump({},open("db/receiptdeposit_template.json", "w"))
			self.template={}
	...
	def AddData(self):
		...
		if self.edit_data !={}:
			if 'data' in self.edit_data:
				...
				self.deposit.setCurrentText(deposit)
	...
	def DepositChange(self,obj):
		...
		#print(self.cashaccountdata)
		index=(self.deposit.currentIndex())
		#print(index)
	...
	def comboboxjournaltype_changed(self,index):
		...
		self.reflineedit.setPlaceholderText(self.autoref)
	...
	def AddJournals(self,item):
		...
		if item.column()==0:
			...
			bal= (self.amountpayable+Dr_  - Cr_)
			
			if (self.amountpayable+Dr_) > Dr_:
	...
	def text_changed_function(self):
		...
		try:
			bal=(parse_decimal(amountpayable,locale='en_US')+ Debit_bal - Credit_bal)
		except Exception as e:
		...
		#print(amountpayable,Debit_bal,Credit_bal)
		if abs(bal)>0:
	...
	def DeleteTemplate(self):
		...
		json.dump(self.template,open("db/receiptdeposit_template.json", "w"))
		self.templatebutton.setText("Use Template {}".format(len(self.template)))
		if len(self.template)==0:
			self.templatebutton.setDisabled(True)
			self.delbtn.setDisabled(True)
	...
	def SaveTemplate(self):
		...
		self.template_data.append(self.deposit.currentText())
		...
		keyname=name+'  '+date
		try:
			template=json.load(open("db/receiptdeposit_template.json","r"))
		...
		json.dump(template,open("db/receiptdeposit_template.json", "w"))	
		self.dialog.close()
	...
	def SaveRecord(self):
			...
			for col in range(cols):
				...
				if col==1:
					...
					if amount=='' or Account=='':
						...
						self.MessageBox.setInformativeText("Amount is empty at row {} 	".format(row+1))
		...
		bal=(amount + Debit_bal - Credit_bal)	
		
		#print((amount , Debit_bal , Credit_bal))
		#print(bal)
		if abs(bal)>0.00001:
			bal=format_currency(bal,'NGN', locale='en_US')
			self.balencelabel.setText('Amount not yet applied to an account (out of balance):  {}'.format(bal))
	...
	def PostJournal(self):
		...
		data.append("receiptdeposit=receiptdeposit&")
	...
	def handleResponse(self, reply):
	    ...
	    if er == QtNetwork.QNetworkReply.NoError:
	        ...
	        if json_ar['19']=='Success':
	        	...
	        	self.MessageBox.show()
	        	#self.table.clearContents()
	        	self.DepositChange(self.deposit.currentText())
